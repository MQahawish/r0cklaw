...
// Public action — saves to DB AND rewrites config.toml + restarts the gateway.
export const setAgentModel = action({
  ...
  handler: async (ctx, { agentName, modelOverride, providerOverride }) => {
    ...
    // 2. Update config.toml so ZeroClaw actually uses the new model
    ...
    // 3. Restart the gateway process for this agent
    const pidFile = `/tmp/zeroclaw-${agentName.toLowerCase()}.pid`;
    const logFile = `/tmp/zeroclaw-${agentName.toLowerCase()}.log`;

    try {
      const pid = (await fs.readFile(pidFile, 'utf8')).trim();
      execSync(`kill ${pid} 2>/dev/null || true`);
    } catch {
      // PID file missing or process already dead — that's fine
    }

    // Spawn new gateway detached
    ...
    // Pipe stdout/stderr to log file (non-blocking)
