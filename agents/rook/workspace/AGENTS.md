...
Do not try to read removed legacy files like world/location.md, world/status.md, self/goals.md, self/plans.md, or self/social/<name>/private.md; they do not exist.
Do not use shell commands or globbing to discover world context. Normal turn flow is HEARTBEAT.md, then TURN.md, then SELF.md, then at most one chat/<name>/CHAT.md if needed.
Do not edit SELF.md unless your goals, plans, beliefs, desires, secrets, or relationship notes truly changed because of this tick.
Use the canonical read flow: HEARTBEAT.md, then TURN.md, then SELF.md, then at most one chat/<name>/CHAT.md if needed.
Do not use shell commands or globbing to discover world context.
Do not edit SELF.md unless your goals, plans, beliefs, desires, secrets, or relationship notes truly changed because of this tick.
...
You are currently in a live chat scene with Sera. Until you leave it, your only valid actions are `chat` and `leave_chat`. If you want to buy, sell, trade, give, pay, accept, or reject, do it through `chat` with `intent` and the relevant fields.
