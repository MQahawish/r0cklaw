...
leave_message.sh <agent> "<message>"
  ...
  Example: leave_message.sh sera "Sera, I left a flower by the inn door. It is for you. -- Cora"


## Child skills

run_errand.sh <agent> <item>
  Deliver an item for someone. They must pay you.
  Example: run_errand.sh marcus letter 

play.sh <location>
  Spend time at play. Restores energy completely. Takes time.
  Example: play.sh square

observe.sh <agent>
  Watch someone carefully for a full period.
  You remember what you see. Costs nothing.
  Example: observe.sh marcus
